...
export default function Projectcard({ src, title, description, href, delay }: { src: string, title: string, description: string, href: string, delay: number }) {
    return (
        <>
            ...
            >
                <Link className={`block h-full bg-slate-950 w-full rounded-md overflow-hidden border-2 border-orange-700 hover:-translate-y-1 duration-300`} draggable={false} href={href} target={href !== '#' ? '_blank' : ''}>
                    <img draggable={false} className='border-b-2 border-orange-700' alt="image" src={src}></img>
